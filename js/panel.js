$(document).ready(() => {
    ...
    function updateGuild() {
        ...
        if (guilds.val() === "DM") {
            ...
            html += `<br>Właściciel: ${guild.owner.user.tag} <button value="<@!${guild.owner.user.id}>" class="mini" onclick="addText(this.value)">@</button><br>`;
            html += `Użytkowników : ${guild.members.cache.filter((member) => !member.user.bot).size}<br>`;
            html += `Kanałów (głosowych) : ${guild.channels.cache.filter((chan) => chan.type === "voice").size}<br>`;
            html += `Kanałów (tekstowych) : ${guild.channels.cache.filter((chan) => chan.type === "text").size}<br><br>`;
            ...
            html += "<button onclick='toggleVisibilityHeight(`#guildMembers`)'>Użytkownicy</button>";
            html += `<div id="guildMembers" style="display:none; opacity: 0;">${guildMembers.join("<br>")}</div>`;

            // Roles button
            html += "<button onclick='toggleVisibilityHeight(`#guildRoles`)'>Rol</button>";
            ...
            if (guild.channels.cache.size > 0) {
                html += "<button onclick='toggleVisibilityHeight(`#guildChannels`)'>Kanały</button>";
